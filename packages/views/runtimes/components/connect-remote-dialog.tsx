...
const CLOUD_SERVER_URL = "https://forge.asymbl.app";
const CLOUD_APP_URL = "https://forge.asymbl.app";
...
function daemonCommands(serverUrl: string | undefined, appUrl: string | undefined) {
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `forge setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}`,
      tokenCmd: `forge config set server_url ${normalizedServerUrl}
forge config set app_url ${normalizedAppUrl}
...
export function ConnectRemoteDialog({ onClose }: { onClose: () => void }) {
  ...
  // `forge setup` is one blocking command that handles config + login
