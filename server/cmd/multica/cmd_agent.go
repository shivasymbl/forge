...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, MULTICA_SERVER_URL env, or 'forge config set server_url <url>'")
	}
...
func resolveServerURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'forge setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
// ~/.forge/config.json would let the agent act on whatever workspace the
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		...
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set MULTICA_WORKSPACE_ID env, or run 'forge config set workspace_id <id>'")
	}
	return id, nil
}
