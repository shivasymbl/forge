...
var loginCmd = &cobra.Command{
	...
	Long: "Log in to Forge, then automatically discover and watch all your workspaces.",
	// Up to one positional is accepted so `--token forge_...` (space form) can
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if err := autoWatchWorkspaces(cmd); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'forge workspace list' and 'forge workspace watch <id>' to set up manually.\n")
		return nil
	}

	fmt.Fprintf(os.Stderr, "\n→ Run 'forge daemon start' to start your local agent runtime.\n")
	return nil
}
...
func waitForWorkspaceCreation(cmd *cobra.Command, client *cli.APIClient) ([]struct {
	...
	if appURL == "" {
		...
		fmt.Fprintln(os.Stderr, "\nNo workspaces found.")
		fmt.Fprintln(os.Stderr, "Create a workspace in the web dashboard, then run 'forge login' again.")
		return nil, nil
	}
