...
// `no such host` errors when calling out (for example, `forge issue get`
// hitting the Forge API). See upstream issue openai/codex#10390.
//
...
// actually reach the Forge API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the forge CLI can reach the API.
...
const (
	multicaManagedBeginMarker = "# BEGIN forge-managed (do not edit; regenerated by daemon)"
	multicaManagedEndMarker   = "# END forge-managed"
)
...
// upsertMulticaManagedBlock returns the config content with the forge-managed
...
// ensureCodexSandboxConfig writes the forge-managed sandbox block into the
