...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"`forge <command> --help`",
		"forge issue get <id> --output json",
		"forge issue comment list <issue-id>",
		"forge issue create --title",
		"forge issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"forge repo checkout <url>",
		"forge issue status <id> <status>",
		"forge issue comment add <issue-id>",
	...
	for _, banned := range []string{
		"forge issue list [--status",
		"forge issue label list",
		"forge issue subscriber list",
		"forge label list",
		"forge workspace member list",
		"forge agent list",
		"forge squad list",
		"forge issue runs",
		"forge issue run-messages",
		"forge attachment download",
		"forge autopilot list",
		"forge autopilot create",
		"forge autopilot update",
		"forge autopilot trigger",
		"forge autopilot delete",
		"forge project get",
		"forge project resource list",
		"forge issue assign",
		"forge issue label add",
		"forge issue label remove",
		"forge issue subscriber add",
		"forge issue subscriber remove",
		"forge issue comment delete",
		"forge label create",
	} {
...
// heredoc on `forge issue create`, the flags were silently swallowed into
...
// non-ASCII as `?` before reaching `forge.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
func TestInjectRuntimeConfigCommentTriggerColdStartRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"forge issue comment list " + issueID + " --thread " + triggerID + " --tail 30 --output json",
		"forge issue comment list " + issueID + " --recent 10 --output json",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "forge issue comment list "+issueID+" --thread "+triggerID+" --output json") {
...
func TestInjectRuntimeConfigCommentTriggerResumedNoDeltaRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"forge issue comment list " + issueID + " --thread thread-root-1 --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
...
func TestInjectRuntimeConfigAssignmentTriggerMentionsRecent(t *testing.T) {
	...
	for _, want := range []string{
		"forge issue comment list issue-1 --recent 10 --output json",
	...
	for _, banned := range []string{
		"forge issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		"forge issue metadata list <issue-id>",
		"forge issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"forge issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `forge issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"forge issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"forge issue metadata set",
				"forge issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_content_file", func(t *testing.T) {
		...
		if !strings.Contains(s, "forge issue metadata list issue-md-codex --output json") {
