	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				"`--status backlog` = **wait**",
				"`forge issue status <child-id> todo`",
				"all `--status todo`",
...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"forge issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`forge issue status <this-issue-id> in_review`",
...
// dedicated protocol step that wrote `forge issue status <this-issue-id> in_review`;
...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	if strings.Contains(out, "`forge issue status <this-issue-id> in_review`") {
...
func TestCommentTriggeredBriefCarriesNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "forge issue comment list "+issueID+" --since "+since+" --output json") {
		t.Errorf("comment brief must keep the issue-wide --since catch-up fallback, got:\n%s", out)
	}
	// The removed resolve step must not reappear.
	if strings.Contains(out, "forge comment resolve") {
...
func TestCommentTriggeredBriefColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "forge issue comment list "+issueID+" --thread thread-root-1 --tail 30 --output json") {
...
func TestAssignmentTriggeredProtocolHonorsAgentIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Run `forge issue status " + issueID + " in_progress` unless your Agent Identity forbids issue status changes; if it does, skip this step.",
		...
		"When done, run `forge issue status " + issueID + " in_review` unless your Agent Identity forbids issue status changes; if it does, skip this step.",
		"If blocked, run `forge issue status " + issueID + " blocked` unless your Agent Identity forbids issue status changes.",
	} {
		if !strings.Contains(out, want) {
	...
	for _, banned := range []string{
		"4. Run `forge issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `forge issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestChatOutputDoesNotRequireIssueComment(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"do not call `forge issue comment add`",
...
func TestWriteRuntimeConfigFileCreatesMissingFile(t *testing.T) {
	...
	const brief = "# Forge Agent Runtime\n\nbrief body line"
