...
func buildQuickCreatePrompt(task Task) string {
	...
	b.WriteString("A user pressed the quick-create shortcut and typed a one-line description. There is NO existing issue. Your job is to create a well-formed issue from the user's input with a single `forge issue create` command.\n\n")
	...
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\"), call `forge workspace members --output json` and find the matching member by display name (case-insensitive substring match is fine). On a clean match, pass `--assignee <name>`. On no match or ambiguous match, do NOT pass `--assignee` — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	agentName := ""
	if task.Agent != nil {
	...
	b.WriteString("- Run exactly one `forge issue create` invocation. Do not retry it for any reason — even on a non-zero exit. The issue may already exist; another attempt would create a duplicate.\n")
	...
	b.WriteString("- Do NOT call `forge issue get` or `forge issue comment add` for this task — there is no issue to query or comment on prior to creation.\n")
