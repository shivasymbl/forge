...
func TestBuildPromptDefaultMentionsRecent(t *testing.T) {
	...
	for _, s := range []string{
		"forge issue comment list issue-default-1 --recent 10 --output json",
		"Next thread cursor:",
		"--since",
	} {
		if !strings.Contains(out, s) {
	...
	if strings.Contains(out, "forge issue comment list issue-default-1 --output json") {
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "forge issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --output json") {
...
func TestBuildPromptColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "forge issue comment list "+issueID+" --recent 10 --output json") {
