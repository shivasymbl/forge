...
```bash
forge autopilot list --output json
forge autopilot get <autopilot-id> --output json
forge autopilot runs <autopilot-id> --output json
```
...
forge autopilot list --output json
forge autopilot get <autopilot-id> --output json
forge autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
forge autopilot update <autopilot-id> --status active|paused --output json
forge autopilot runs <autopilot-id> --output json
forge autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
forge autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
forge autopilot trigger <autopilot-id> --output json
forge autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `forge autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `forge autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `forge squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `forge agent get <agent-id> --output json` and `forge runtime list --output json`.
