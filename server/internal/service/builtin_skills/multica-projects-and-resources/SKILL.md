---
name: forge-projects-and-resources
...
```bash
forge project list --output json
forge project get <project-id> --output json
...
A project groups work and carries durable resources. A resource is not just display metadata; it is context later injected into task briefs and `.forge/project/resources.json`.

A project's `description` is also durable context: when an issue (or a quick-create task) is bound to a project, the project description is injected into the agent's brief under `## Project Context` and written to `.forge/project/resources.json` as `project_description`. Use it for project-wide rules/context that should apply to every task in the project.
...
forge project list --output json
forge project get <project-id> --output json
forge project create --title "<title>" --repo <github-url> --output json
forge project update <project-id> --title "<title>" --output json
forge project status <project-id> in_progress --output json
forge project resource list <project-id> --output json
...
forge project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --output json
forge project resource update <project-id> <resource-id> --url <new-github-url> --output json
forge project resource update <project-id> <resource-id> --ref <branch-or-sha> --output json
forge project resource remove <project-id> <resource-id> --output json
```
...
Project resources are durable and affect future tasks. `forge repo checkout`
is task-local checkout state.

## Debugging wrong context

1. `forge project get <project-id> --output json`.
2. `forge project resource list <project-id> --output json`.
