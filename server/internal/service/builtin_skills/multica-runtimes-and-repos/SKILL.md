...
7. `forge repo checkout` talks to the local daemon, not directly to GitHub.

## CLI

```bash
forge runtime list --output json
forge runtime usage <runtime-id> --output json
forge runtime activity <runtime-id> --output json
forge runtime update <runtime-id> --target-version <version> --output json
forge runtime delete <runtime-id>
forge repo checkout <url>
forge repo checkout <url> --ref <branch-or-sha>
```
...
4. Is the runtime online? `forge runtime list --output json`.
